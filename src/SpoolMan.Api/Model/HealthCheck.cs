...
namespace SpoolMan.Api.Model
{
    ...
    public class HealthCheckJsonConverter : JsonConverter<HealthCheck>
    {
        ...
        public override HealthCheck Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (!status.IsSet)
                throw new ArgumentException("Property is required for class HealthCheck.", nameof(status));

            if (status.IsSet && status.Value == null)
                throw new ArgumentNullException(nameof(status), "Property is not nullable for class HealthCheck.");

            return new HealthCheck(status.Value!);
        }
        ...
        public void WriteProperties(Utf8JsonWriter writer, HealthCheck healthCheck, JsonSerializerOptions jsonSerializerOptions)
        {
            if (healthCheck.Status == null)
                throw new ArgumentNullException(nameof(healthCheck.Status), "Property is required for class HealthCheck.");

            writer.WriteString("status", healthCheck.Status);
        }
    }
}
